...
router.post("/wiki/new", async (req, res, next) => {
  try {
    ...
    currentUser.set("wikis_worked_on", [
      ...currentUser.attributes.wikis_worked_on,
      new_wiki_worked_on,
    ]);
...
router.post("/wiki/save", async (req, res, next) => {
  try {
    ...
    currentUser.set("activity_log", [
      ...currentUser.attributes.activity_log,
      newActivity,
    ]);
    // update wikis worked on fields if not already worked on
    if (
      currentUser.attributes.wikis_worked_on.filter(
        (wiki) => wiki.wikiID === wikiID
      ).length === 0
    ) {
      /* User does not contain the current wiki */
      const new_wiki_worked_on = {
    ...
    console.log(
      "✅ Successfully save changes to wiki and added change to user!"
    );
  } catch (error) {
    console.log("error: ", error);
    res.status(400);
...
router.post("/wiki/search", async (req, res, next) => {
  try {
    ...
    const resWikisInfoUpvote = [...resWikisInfoRecency].sort(
      (a, b) => b.attributes.wikiObject.points - a.attributes.wikiObject.points
    );
...
router.get("/home", async (req, res, next) => {
  try {
    ...
    const wikisWorkedOnObjects = await Promise.all(
      wikiIDs.map(async (wikiID) => {
        const queryWiki = new Parse.Query("Wiki");
        queryWiki.equalTo("objectId", wikiID);
        const wikiInfo = await queryWiki.first();
    ...
    const wikiFeeds = await Promise.all(
      wikiIDs.map(async (wikiID) => {
        const queryWiki = new Parse.Query("Wiki");
        queryWiki.equalTo("objectId", wikiID);
        const wikiInfo = await queryWiki.first();
